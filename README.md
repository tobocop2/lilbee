...
<p align="center">
  ...
  <a href="https://github.com/tobocop2/scoop-lilbee"><img src="https://img.shields.io/badge/Scoop-bucket-908caa?logo=windows&logoColor=908caa&style=flat-square&labelColor=191724" alt="Scoop bucket"></a>
...
<p align="center">
  <a href="https://github.com/tobocop2/lilbee/releases"><img src="https://img.shields.io/github/downloads/tobocop2/lilbee/total?label=downloads&logo=github&logoColor=9ccfd8&style=flat-square&labelColor=191724&color=9ccfd8" alt="Release downloads"></a>
...
| **Scoop**             | `scoop install git; scoop bucket add lilbee https://github.com/tobocop2/scoop-lilbee; scoop install lilbee` | Windows x86_64. Scoop needs git to add a bucket, so the first command installs it (a no-op when present). Reads your NVIDIA driver version and installs the matching CUDA build (555.85+ gets cu125, 551.61+ gets cu124), otherwise the Vulkan build. It says which one it picked. `scoop update lilbee` upgrades. Added the bucket before 2026-09-08? It moved to its own repository, so run `scoop bucket rm lilbee` and add it again; your install is untouched. |
| **npm**               | `npm install -g lilbee`                                                                  | macOS / Linux x86_64 / Windows, Node 18+. A small launcher: on first run it detects your hardware (CUDA, ROCm, Metal, or the AVX-baseline build), downloads the matching standalone binary of the latest release, verifies it, and caches it. `lilbee prepare` does that ahead of time; `npx -y lilbee mcp` runs the MCP server for agent hosts. |
...
| **Scoop**    | `scoop install lilbee-cuda` (cu125 in one download, driver 555.85+; the plain `lilbee` install also auto-picks CUDA, but downloads the Vulkan build first)                    |
...
| **Binary + GPU** | [`lilbee-compat-linux-x86_64-cu124`](https://github.com/tobocop2/lilbee/releases/latest) (NVIDIA CUDA, driver 550+) or [`lilbee-compat-linux-x86_64-rocm`](https://github.com/tobocop2/lilbee/releases/latest) (AMD ROCm). These need a CPU with AVX (2011+); the Vulkan build above covers older chips. |
| **pip** (devs) | `pip install --pre lilbee 'lancedb==0.39.0+compat' --extra-index-url https://lilbee.sh/compat/` — no engine: there is no compat engine wheel, so use a bundled build above or bring your own `llama-server` |
...
<details>
<summary><b>All 152 supported model architectures. Click to expand.</b></summary>

| | | | |
|---|---|---|---|
| `afmoe` | `apertus` | `arcee` | `arctic` |
| `arwkv7` | `baichuan` | `bailingmoe` | `bailingmoe2` |
| `bailingmoe3` | `bert` | `bitnet` | `bloom` |
| `chameleon` | `chatglm` | `clip` | `codeshell` |
...
| `grovemoe` | `hrm_text` | `hunyuan-dense` | `hunyuan-moe` |
| `hunyuan_vl` | `hy_v3` | `hy_v4` | `internlm2` |
...
| `mamba` | `mamba2` | `maple` | `mellum` |
| `mimo2` | `minicpm` | `minicpm3` | `minimax-01` |
| `minimax-m2` | `minimax-m3` | `mistral3` | `mistral4` |
| `modern-bert` | `mpt` | `muse-glimmer` | `nanbeige` |
| `nemotron` | `nemotron_h` | `nemotron_h_moe` | `neo-bert` |
| `nomic-bert` | `nomic-bert-moe` | `olmo` | `olmo2` |
| `olmoe` | `openelm` | `orion` | `paddleocr` |
| `pangu-embedded` | `phi2` | `phi3` | `phimoe` |
| `plamo` | `plamo2` | `plamo3` | `plm` |
| `pockettts` | `qwen` | `qwen2` | `qwen2moe` |
| `qwen2vl` | `qwen3` | `qwen35` | `qwen35moe` |
| `qwen3moe` | `qwen3next` | `qwen3tts` | `qwen3vl` |
| `qwen3vlmoe` | `qwen4exp` | `refact` | `rnd1` |
| `rwkv6` | `rwkv6qwen2` | `rwkv7` | `seed_oss` |
| `smallthinker` | `smollm3` | `spark2_5` | `stablelm` |
...
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| PDF             | `.pdf`                                                                                                                                                                                                                                                                                                                       | none; scanned pages need [Tesseract](https://github.com/tesseract-ocr/tesseract) or a vision model |
| Office          | `.doc`, `.docm`, `.docx`, `.dot`, `.dotm`, `.dotx`, `.fodg`, `.hwp`, `.hwpx`, `.key`, `.numbers`, `.odp`, `.ods`, `.odt`, `.pages`, `.pot`, `.potm`, `.potx`, `.pps`, `.ppsx`, `.ppt`, `.pptm`, `.pptx`, `.rtf`, `.wp`, `.wp5`, `.wp6`, `.wpd`, `.xla`, `.xlam`, `.xls`, `.xlsb`, `.xlsm`, `.xlsx`, `.xlt`, `.xltm`, `.xltx` | none                                                                                               |
| eBook           | `.epub`, `.fb2`                                                                                                                                                                                                                                                                                                              | none                                                                                               |
| Images (OCR)    | `.avcs`, `.avif`, `.bmp`, `.gif`, `.heic`, `.heics`, `.heif`, `.heifs`, `.hif`, `.j2c`, `.j2k`, `.jb2`, `.jbig2`, `.jp2`, `.jpc`, `.jpeg`, `.jpg`, `.jpg2`, `.pbm`, `.pgm`, `.png`, `.pnm`, `.ppm`, `.tif`, `.tiff`, `.webp`                                                                                                 | [Tesseract](https://github.com/tesseract-ocr/tesseract) or a vision model                          |
| Notebooks       | `.ipynb`, `.qmd`, `.rmd`                                                                                                                                                                                                                                                                                                     | none                                                                                               |
| Bibliographies  | `.bib`, `.enw`, `.nbib`, `.ris`                                                                                                                                                                                                                                                                                              | none                                                                                               |
| Email           | `.eml`, `.msg`, `.pst`                                                                                                                                                                                                                                                                                                       | none                                                                                               |
| Data            | `.csv`, `.db`, `.dbf`, `.geojson`, `.gpkg`, `.gpkx`, `.json`, `.jsonl`, `.kml`, `.ndjson`, `.sqlite`, `.sqlite3`, `.toml`, `.tsv`, `.xml`, `.yaml`, `.yml`                                                                                                                                                                   | none                                                                                               |
| Text and markup | `.adoc`, `.asciidoc`, `.commonmark`, `.dbk`, `.dj`, `.djot`, `.docbook`, `.docbook4`, `.docbook5`, `.doctags`, `.htm`, `.html`, `.jats`, `.latex`, `.markdown`, `.md`, `.mdx`, `.nxml`, `.opml`, `.org`, `.rst`, `.tex`, `.txt`, `.typ`, `.typst`, `.vtt`, `.xht`, `.xhtml`                                                  | none                                                                                               |
| Archives        | `.7z`, `.gz`, `.tar`, `.tgz`, `.zip` (each file inside is indexed as its own document, named `archive.zip/file.pdf`)                                                                                                                                                                                                         | none                                                                                               |
| Code            | `.py`, `.js`, `.ts`, `.go`, `.rs`, `.java`, `.c`, `.cpp`, `.rb` and [306 languages](https://github.com/Goldziher/tree-sitter-language-pack) via tree-sitter (AST-aware chunking)                                                                                                                                             | none                                                                                               |
<!-- formats-table:end -->

See the [usage guide](docs/usage.md#ocr) for OCR setup.
