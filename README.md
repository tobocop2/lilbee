...
| **Scoop**             | `scoop install git; scoop bucket add lilbee https://github.com/tobocop2/lilbee; scoop install lilbee` | Windows x86_64. Scoop needs git to add a bucket, so the first command installs it (a no-op when present). Reads your NVIDIA driver version and installs the matching CUDA build (555.85+ gets cu125, 551.61+ gets cu124), otherwise the Vulkan build. It says which one it picked. `scoop update lilbee` upgrades. |
