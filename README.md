...
| **[lilbee](https://github.com/tobocop2/lilbee/releases)** (ROCm, opt-in for AMD) | n/a | n/a | 1.28 GB* | the same whole stack, with the faster ROCm runtime |
...
\* ROCm is the one figure not yet from a release: it's the engine wheel CI builds today. The standalone binary lands here at the first release that carries it.
...
| How                   | Command                                                                                  | Notes                                                                                                                                                                                                                 |
| --------------------- | ---------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **pip**               | `pip install --pre lilbee`                                                               | Recommended. The default wheel runs on any x86_64 CPU with AVX2 (2013+; older CPUs: [On older CPUs](#on-older-cpus-pre-avx2)) and uses your GPU via Vulkan / Metal automatically. Faster vendor builds: [CUDA](#on-nvidia-hardware), [ROCm](#on-amd-hardware). Intel Mac: add `--extra-index-url https://lilbee.sh/cpu/` ([browse wheels](https://lilbee.sh/cpu/lilbee/)). |
...
Same `lilbee` command after install. Linux only. Cards: MI50, MI100, MI200, MI300, RDNA2 (gfx1030) and RDNA3 (gfx1100/1101/1102). Largest of the three builds, since the ROCm userspace and per-card kernels ship inside it.
