...
def lint(manifest: dict, tapes_dir: pathlib.Path, pending: set[str]) -> list[str]:
    ...
    for reel_name, reel in manifest["reels"].items():
        ...
        # VHS 0.10.0 rejects Ctrl+s (XOFF) and Ctrl+q (XON) at parse time
        for n, line in enumerate(body, start=len(expected) + 1):
            if line.strip() in ("Ctrl+s", "Ctrl+q", "Ctrl+S", "Ctrl+Q"):
                findings.append(f"{reel_name}: {line.strip()} is not a valid VHS key (flow control) at line {n}")
