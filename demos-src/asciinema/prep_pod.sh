...
refresh_checkout() {
  # demos/final-reels = latest origin/main + the search overhaul (#508,
  # fix/hybrid-ann-recall) merged in. #508 never landed on main, so we
  # reproduce the exact local merge from two already-pushed origin refs
  # (deterministic; the merge was verified clean, zero conflicts).
  cd /workspace/lilbee
  git fetch origin main fix/hybrid-ann-recall
  git checkout -B demos/final-reels origin/main
  git -c user.email=demos@lilbee -c user.name=demos merge --no-edit origin/fix/hybrid-ann-recall
