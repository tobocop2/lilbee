...
# palette sanity is a THEME-LEAK detector, not a per-UI exact match. Each agent
# UI has its own dark bg (opencode renders a near-black ~9/9/9, a bare terminal
# ~23/23/23, hermes/godot their own darks); pinning one exact value false-fails
# every UI not physically GPU-measured. The only bg that is both dark AND
# blue-shifted is the rose-pine base, so that single signature cleanly separates
# themed lilbee-TUI reels from agent reels and catches the real failures: a
# rose-pine bg bleeding onto an agent reel, a lilbee reel that lost its theme,
# and any light/tofu background.
BG_MAX_DARK = 60         # any real bg here (rose-pine 35, opencode 9, term 23) is < this; light/tofu > this
BG_BLUE_SHIFT = 4        # rose-pine base B-R ~10; neutral agent darks ~0
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # themed = rose-pine lilbee TUI; everything else is an agent/terminal UI
        themed = reel.get("bg_target", "rose-pine" if cls.get("theme") else "agent") == "rose-pine"
        is_dark = bg.max() < BG_MAX_DARK
        blue_shift = bg[2] - bg[0]
        if themed and not (is_dark and blue_shift >= BG_BLUE_SHIFT):
            findings.append(f"bg {rep['mid_bg']} not the rose-pine base (theme failed to load?)")
        elif not themed and not (is_dark and blue_shift < BG_BLUE_SHIFT):
            findings.append(f"bg {rep['mid_bg']} not a dark-neutral agent bg (rose-pine leak / wrong theme?)")
