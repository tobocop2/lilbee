...
def candidate_fits(manifest: dict, group: str, gpu: tuple[str, int],
    ...
    for key in keys:
        ...
        if not row:
            ...
            continue
        # the fleet co-locates the embedder (Qwen3-Embedding-8B-Q8_0, ~9GB) on
        # the same device as the chat model — warm.sh's RAG ask loads both — so
        # the per-device budget must reserve for it. +3 underestimated it and
        # let gptoss-120b (71.7GB) pick an 80GB card it then OOMs on at warm.
        need = row["max_device_gb"] + 10.0   # co-resident embedder + KV headroom
        if need > dev_gb * 0.95:
