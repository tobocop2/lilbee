...
# Pod defaults, overridable so the same staging runs on the laptop. The laptop wave
# needs identical behaviour to the pod wave or the two produce different reels: the
# empty-lancedb rule and the model re-registration below are the parts that bite.
DATA = pathlib.Path(os.environ.get("REEL_DATA", "/root/demo-data"))
KB_SRC = pathlib.Path(os.environ.get("REEL_KB", "/root/kb"))
