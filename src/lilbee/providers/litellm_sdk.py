...
class LitellmSdkBackend:
    ...
    def rerank(self, request: RerankRequest) -> RerankResult:
        """Rerank documents through ``litellm.rerank``.

        Works with every provider LiteLLM exposes a rerank endpoint for
        (Cohere, Voyage, Jina, Together AI, HF TEI). The response's
        ``results`` array is ordered by descending relevance; we restore
        input order via each result's ``index`` so the caller can
        correlate scores with its ``candidates`` list.
        """
