...
overridden. One resolver feeds both seams (the launch argv and the
``supports_tools`` decision), so the two cannot disagree.
"""
...
# GGUF ``general.architecture`` -> vendored tool-aware template filename. Each
# vendored template is the model's upstream template, with any ``tojson`` guarded
# so the no-tools warmup call does not crash llama-server's jinja engine. Register
# an arch only once verified end-to-end to emit a structured, dispatchable tool
# call (an override that offers tools the engine cannot parse is worse than none).
