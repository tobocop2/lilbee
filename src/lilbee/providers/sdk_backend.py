...
@dataclass(frozen=True)
class RerankRequest:
    """Backend-agnostic request for a rerank call.

    ``query`` is the search string; ``candidates`` are the documents to
    score. The backend returns one float per candidate in input order;
