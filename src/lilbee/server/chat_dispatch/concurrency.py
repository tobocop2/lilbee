...
async def acquire_chat_lock_or_busy(timeout: float | None = None) -> None:
    ...
    try:
        ...
        rendered = (
            f"{effective_timeout:.1f}s" if effective_timeout < 1 else f"{effective_timeout:.0f}s"
        )
        raise ChatBusyError(
            f"Chat backend busy: another request held the lock for {rendered}. Retry shortly."
        ) from exc
