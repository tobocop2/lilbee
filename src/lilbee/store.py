...
# ---------------------------------------------------------------------------
# Backwards-compatible module-level API (delegates to a default Store instance)
# ---------------------------------------------------------------------------
...
def _get_default_store() -> Store:
    ...
    if _default_store is None:
        _default_store = Store(cfg)
    return _default_store


def _reset_default_store() -> None:
    """Reset the cached default store (for tests that mutate cfg)."""
    global _default_store
    _default_store = None


# Module-level _fts kept for backwards compat in tests that check store._fts
_fts = _FtsState()


def get_db() -> lancedb.DBConnection:
    return _get_default_store().get_db()


def open_table(name: str) -> lancedb.table.Table | None:
    """Open a table if it exists, otherwise return None."""
    return _get_default_store().open_table(name)


def _chunks_schema() -> pa.Schema:
    return _get_default_store()._chunks_schema()


def ensure_fts_index() -> None:
    s = _get_default_store()
    s.ensure_fts_index()
    _fts.ready = s._fts.ready


def add_chunks(records: list[dict]) -> int:
    s = _get_default_store()
    result = s.add_chunks(records)
    _fts.ready = s._fts.ready
    return result
...
def bm25_probe(query_text: str, top_k: int = 5) -> list[SearchChunk]:
    s = _get_default_store()
    result = s.bm25_probe(query_text, top_k)
    _fts.ready = s._fts.ready
    return result
...
def search(
    ...
    s = _get_default_store()
    result = s.search(query_vector, top_k, max_distance, query_text)
    _fts.ready = s._fts.ready
    return result
...
def _adaptive_filter(
    ...
    return _get_default_store()._adaptive_filter(results, top_k, initial_threshold)


def get_chunks_by_source(source: str) -> list[SearchChunk]:
    return _get_default_store().get_chunks_by_source(source)


def delete_by_source(source: str) -> None:
    _get_default_store().delete_by_source(source)


def delete_source(filename: str) -> None:
    _get_default_store().delete_source(filename)


def get_sources() -> list[SourceRecord]:
    return _get_default_store().get_sources()


def upsert_source(filename: str, file_hash: str, chunk_count: int) -> None:
    _get_default_store().upsert_source(filename, file_hash, chunk_count)
...
def remove_documents(
    ...
    *,
    ...
    """Remove documents -- calls module-level functions for mock compatibility."""
    if documents_dir is None:
