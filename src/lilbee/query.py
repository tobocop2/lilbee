...
class Searcher:
    """RAG search pipeline — delegates to module-level functions during migration.

    Holds references to config, provider, store, and embedder for composition.
    Methods delegate to the canonical module-level implementations which are
    fully tested. Once all entry points are wired through Searcher, the
    module-level functions become the thin wrappers instead.
    """
