...
@contextmanager
def _streaming(stream: _HeldStream):
    """Patch the answer stream and release it before the app can shut down.

    Entered inside ``app.run_test()`` so the release always reaches a live
    worker, on every exit path, including a failed assertion.
    """
...
async def test_a_stale_turn_leaves_the_resumed_chat_alone(held):
    ...
    app = LilbeeApp()
    async with app.run_test(size=(120, 40)) as pilot:
        with _streaming(held), patch.object(ChatScreen, "_embedding_ready", return_value=True):
            screen = await _start_held_turn(app, pilot, held)
