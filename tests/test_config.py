...
class TestFromEnvBackwardsCompat:
    """Config.from_env() still works as a factory method."""

    def test_from_env_returns_config(self):
        c = Config.from_env()
        assert isinstance(c, Config)
        assert c.chat_model == "qwen3:8b" or c.chat_model  # has a value
...
class TestLitellmBaseUrl:
    def test_ollama_host_fallback(self):
        env = _clean_env()
        env["OLLAMA_HOST"] = "http://myhost:11434"
        with mock.patch.dict(os.environ, env, clear=True):
            c = Config()
            assert c.litellm_base_url == "http://myhost:11434"

    def test_lilbee_env_overrides_ollama_host(self):
        with mock.patch.dict(
            os.environ,
            {
                "LILBEE_LITELLM_BASE_URL": "http://explicit:11434",
                "OLLAMA_HOST": "http://ollama:11434",
            },
        ):
            c = Config()
            assert c.litellm_base_url == "http://explicit:11434"
