...
class TestRoutingProvider:
    ...
    def test_routes_count_chat_prompt_tokens_to_local_engine_for_local_ref(self) -> None:
        rp = self._make_provider()
        mock_llama = mock.MagicMock()
        mock_llama.count_chat_prompt_tokens.return_value = 31
        rp._local = mock_llama
        messages = [{"role": "user", "content": "some text"}]

        cfg.chat_model = "vendor/Model-GGUF/model-Q4_K_M.gguf"
        assert rp.count_chat_prompt_tokens(messages) == 31
        mock_llama.count_chat_prompt_tokens.assert_called_once_with(
            messages, options=None, model=None, tools=None, tool_choice=None
        )

    def test_routes_count_chat_prompt_tokens_to_sdk_for_remote_ref(self) -> None:
        rp = self._make_provider()
        mock_sdk = mock.MagicMock()
        mock_sdk.count_chat_prompt_tokens.side_effect = NotImplementedError
        rp._sdk_provider = mock_sdk

        cfg.chat_model = "ollama/qwen3:8b"
        with pytest.raises(NotImplementedError):
            rp.count_chat_prompt_tokens([{"role": "user", "content": "some text"}])
...
class TestLitellmForkGuard:
    ...
    @pytest.mark.parametrize(
        "versions",
        [{"litellm": "1.98.0"}, {"unclecode_litellm": "1.81.13"}, {}],
        ids=["litellm-only", "fork-only", "neither"],
    )
    def test_silent_unless_both_are_installed(self, versions: dict[str, str]) -> None:
        from lilbee.providers.litellm_sdk import _require_litellm
...
def test_sdk_provider_count_chat_prompt_tokens_not_implemented() -> None:
    """Cloud SDK backends render no prompt locally, so the count route estimates instead."""
    from lilbee.providers.litellm_sdk import LitellmSdkBackend
    from lilbee.providers.sdk_llm_provider import SdkLLMProvider
