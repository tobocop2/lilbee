"""Tests for `lilbee agent-config cline`."""
...
def test_cline_config_emits_anthropic_flavor_block():
    _write_server_session("cline-token", 8765)
    ...
    with patch(
        ...
        result = runner.invoke(app, ["agent-config", "cline"])

    assert result.exit_code == 0, result.stderr
    payload = json.loads(result.stdout)
    assert payload["apiProvider"] == "anthropic"
    assert payload["apiModelId"] == _CHAT_REF_A
    assert payload["anthropicBaseUrl"] == "http://127.0.0.1:8765"
    assert payload["apiKey"] == "cline-token"


def test_cline_config_picks_first_chat_model_alphabetically():
    ...
    assert result.exit_code == 0, result.stderr
    payload = json.loads(result.stdout)
    # _CHAT_REF_B starts with "bartowski/..." which sorts before "Qwen/..." in ASCII
    assert payload["apiModelId"] == sorted([_CHAT_REF_A, _CHAT_REF_B])[0]


def test_cline_config_without_chat_model_still_emits_block():
    _write_server_session("tok", 9000)
    ...
    payload = json.loads(result.stdout)
    assert payload["apiProvider"] == "anthropic"
    assert payload["apiModelId"] == ""
    assert payload["anthropicBaseUrl"] == "http://127.0.0.1:9000"


def test_cline_config_without_running_server_exits_1():
    result = runner.invoke(app, ["agent-config", "cline"])
