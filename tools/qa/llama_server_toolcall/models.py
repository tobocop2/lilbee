...
ROSTER: tuple[ModelSpec, ...] = (
    ...
    # --- giant tier, multi-GPU (run explicitly via --families) ---
    ModelSpec(
        family="minimax-m2",
        repo="unsloth/MiniMax-M2-GGUF",
        gguf="Q4_K_M/MiniMax-M2-Q4_K_M-00001-of-00003.gguf",
        allow_patterns=("Q4_K_M/MiniMax-M2-Q4_K_M-*.gguf",),
        template_repo="MiniMaxAI/MiniMax-M2",
        multi_gpu_only=True,
        inprocess="never run in-process (needs multi-GPU)",
        notes="~230B, Q4_K_M ~138GB, 3 shards",
    ),
    ModelSpec(
        family="qwen3-235b",
        repo="unsloth/Qwen3-235B-A22B-Instruct-2507-GGUF",
        gguf="Q4_K_M/Qwen3-235B-A22B-Instruct-2507-Q4_K_M-00001-of-00003.gguf",
        allow_patterns=("Q4_K_M/Qwen3-235B-A22B-Instruct-2507-Q4_K_M-*.gguf",),
        multi_gpu_only=True,
        inprocess="never run in-process (needs multi-GPU)",
        notes="235B-A22B, Q4_K_M ~142GB, 3 shards; qwen3 template is tool-aware",
    ),
    ModelSpec(
        family="glm-4.6",
        repo="unsloth/GLM-4.6-GGUF",
        gguf="Q4_K_M/GLM-4.6-Q4_K_M-00001-of-00005.gguf",
        allow_patterns=("Q4_K_M/GLM-4.6-Q4_K_M-*.gguf",),
        template_repo="zai-org/GLM-4.6",
        multi_gpu_only=True,
        inprocess="never run in-process (needs multi-GPU)",
        notes="357B, Q4_K_M ~200GB, 5 shards; glm46 format",
    ),
)
