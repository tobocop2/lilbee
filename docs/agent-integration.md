...
|------|-------------|---------------------|
| `search(query, top_k)` | Search for relevant document chunks by vector similarity | No (uses pre-computed embeddings) |
| `status()` | Show indexed documents, config, and chunk counts | No |
| `sync()` | Sync documents directory with the vector store | Yes (for embedding) |
| `add(paths, force, vision_model)` | Add files, dirs, or URLs and sync them into the vector store | Yes (for embedding) |
| `crawl(url, depth, max_pages)` | Start a non-blocking crawl of a web page (or site); returns task_id for polling | No (crawl only; sync separately) |
| `crawl_status(task_id)` | Check status of a running crawl task (pages crawled, errors, completion) | No |
| `init(path)` | Initialize a local `.lilbee/` knowledge base in a directory | No |
| `remove(names, delete_files)` | Remove documents from the index (optionally delete source files) | No |
| `list_documents()` | List all indexed documents with chunk counts | No |
| `reset()` | Delete all documents and data (factory reset) | No |
| `model_list(source, task)` | List installed models, optionally filtered by source or task | No |
| `model_show(model)` | Show catalog and installed metadata for a model ref | No |
| `model_pull(model, source)` | Download a model, streaming progress via MCP progress notifications | Yes (download) |
