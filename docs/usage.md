...
| `LILBEE_SEMANTIC_CHUNKING` | `true` | Use topic-aware semantic chunking during ingest. Kreuzberg downloads a small ONNX embedding model on first use to detect topic boundaries (separate from the chunk-to-vector embedder). Set `false` for fixed-size splitting with no extra download. |
