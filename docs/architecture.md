...
- Token counting: `POST /v1/messages/count_tokens` answers `{"input_tokens": N}` for the same body `/v1/messages` takes, minus the sampling fields. The system prompt, the conversation, and the tool schemas go to the chat server as the body a chat call would send; the server's `/apply-template` renders the prompt and its `/tokenize` counts it, so the number includes the template's role markers and tool-call preamble. Counting runs no tool, so the route does not apply the tool-capability check `/v1/messages` runs. It is not free of generation either: the first count against a fresh replica sends the one-token exchange that probes whether the template accepts consecutive same-role turns, and later counts reuse that verdict. A model with no local tokenizer (a remote provider, or an engine without the template route) answers with a high estimate rather than an error, because an under-count makes a client overflow its window. Every answer carries the `X-Lilbee-Token-Count-Accuracy` response header, `exact` when the backend measured the number and `estimated` when it did not, so a client can tell the two apart while the response body keeps the one field Anthropic documents. The header says estimated rather than minimum on purpose: the allowances below model llama.cpp template overhead, and the backends that reach this path format the prompt on the provider's own server, so the estimate can land either side of the truth. The estimate counts the request's own text in UTF-8 bytes, which no token encodes fewer than one of, and adds four fixed token allowances for the template text the request does not carry: the preamble a template renders around any request, the role markers around each message the provider is sent, the tool-calling instructions a template emits once when tools are present, and the wrapper around each tool schema. A message that returns several tool results becomes several messages on the wire, so the per-message allowance is charged against that count rather than against the count the client sent. The allowances are sized above the widest values measured on the Qwen3 and SmolLM3 templates, with headroom. They are a heuristic, not a proven bound: a template can substitute a larger preamble than any fixed allowance covers, which is what a byte count of the request alone cannot see. A chars-per-token ratio is a different failure: it is an average and falls short on dense input.
