...
- Token counting: `POST /v1/messages/count_tokens` answers `{"input_tokens": N}` for the same body `/v1/messages` takes, minus the sampling fields. The system prompt, the conversation, and the tool schemas go to the chat server as the body a chat call would send; the server's `/apply-template` renders the prompt and its `/tokenize` counts it, so the number includes the template's role markers and tool-call preamble. Counting runs no tool and generates nothing, so the route does not apply the tool-capability check `/v1/messages` runs. A model with no local tokenizer (a remote provider, or an engine without the template route) answers with an upper bound rather than an error, because an under-count makes a client overflow its window. The bound is the UTF-8 byte length of the request's text: no token encodes fewer than one byte, so the byte count is a ceiling whatever the tokenizer merges, where a chars-per-token ratio is an average and falls short on dense input.
