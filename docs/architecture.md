...
- Token counting: `POST /v1/messages/count_tokens` answers `{"input_tokens": N}` for the same body `/v1/messages` takes, minus the sampling fields. The system prompt, the conversation, and the tool schemas are flattened through the same wire translation the chat call sends, then tokenized by the chat server's own `/tokenize`, so the number is the engine's rather than a character estimate. A model served by a remote provider has no tokenizer to ask, so it answers 400 in the same envelope.
