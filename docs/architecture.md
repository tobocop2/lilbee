...
- **Listing**: "what files can you see" names the indexed documents from the source table, capped at 50 with the total stated. Retrieval answers this from the sources of its top-k chunks, capped again per source, so a library of hundreds reads as about seven documents.
...
The rewrite is validated before retrieval uses it. A small model answers the condense prompt with a lead-in ("Sure, here is the standalone query:"), a refusal, or its reasoning as readily as with the query, and an unchecked reply sends retrieval after text the user never typed. Each reply line must be one sentence, stay near the question's length, and share a content word with the question or the history it refers to; a line failing that falls back to the question as typed. The checks are punctuation and the turn's own vocabulary, not a list of known lead-ins, so they hold in any language whose pack supplies stopwords. When a rewrite is accepted, the query retrieval actually ran is logged and reported to the client: `retrieval_query` on the ask/chat response, and a `retrieval_query` SSE event ahead of the answer on the streams.
