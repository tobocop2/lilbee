...
- Token counting: `POST /v1/messages/count_tokens` answers `{"input_tokens": N}` for the same body `/v1/messages` takes, minus the sampling fields. The system prompt, the conversation, and the tool schemas go to the chat server as the body a chat call would send; the server's `/apply-template` renders the prompt and its `/tokenize` counts it, so the number includes the template's role markers and tool-call preamble. Counting runs no tool, so the route does not apply the tool-capability check `/v1/messages` runs. It is not free of generation either: the first count against a fresh replica sends the one-token exchange that probes whether the template accepts consecutive same-role turns, and later counts reuse that verdict. A model with no local tokenizer answers with an estimate instead of an error; every answer carries the `X-Lilbee-Token-Count-Accuracy` response header, `exact` when the backend measured the number and `estimated` when it did not, so a client can tell the two apart while the response body keeps the one field Anthropic documents. The estimate counts the request's own text in UTF-8 bytes and adds four fixed token allowances for the template text the request does not carry: the preamble a template renders around any request, the role markers around each message the provider is sent, the tool-calling instructions a template emits once when tools are present, and the wrapper around each tool schema. A message that returns several tool results becomes several messages on the wire, so the per-message allowance is charged against that count rather than against the count the client sent. The allowances are constants sized from a survey of 47 chat templates across 51 repositories, taken 2026-09-18. Four of those templates exceed them: Devstral-Small-2507's preamble runs 1216 tokens against an allowance of 300; Falcon3's tool block runs 608 and Llama-4-Scout's runs 514, both against an allowance of 110; Falcon3's per-tool wrapper runs 45 against an allowance of 32; and Apriel-1.5's per-message cost runs 17 against an allowance of 8. The estimate can read low as well as high, and raising a constant only moves which template breaks it. The population that reaches this path is cloud SDK backends, whose prompt formatting happens on the provider's own server, which lilbee cannot see; that is why the header says estimated rather than minimum, and the estimate can land either side of the truth. A chars-per-token ratio is a different failure: it is an average and falls short on dense input.
