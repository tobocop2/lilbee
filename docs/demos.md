...
(810 XML files) via `lilbee-worker`, then `lilbee_search`-es for `AStarGrid2D`,
`TileMapLayer`, and friends as it writes a procedural level generator. Every API call is
backed by a citation into the actual class XML. See
[`docs/benchmarks/godot-level-generator.md`](benchmarks/godot-level-generator.md) for the
side-by-side against the no-RAG baseline (4 hallucinated APIs without lilbee, 0 with).
